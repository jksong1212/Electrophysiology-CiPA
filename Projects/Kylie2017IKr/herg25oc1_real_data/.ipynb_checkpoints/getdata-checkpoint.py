...
def get_dataset(cell_to_remove=[], scale=1, multi=False, use_torch=False):        
    dataset_dir = os.path.dirname(os.path.realpath(__file__)) 
    full_cell_li = []
    ...
    if use_torch:
        ...
        if multi and len(cell_li)>1:                
            ...
            for i, fileNo in enumerate(cell_li):
                I, P = get_data(scale, fileNo)   
                currents.append(torch.tensor(I))
                parameters.append(torch.tensor(P))
        return torch.cat(currents), torch.cat(parameters), cell_li
        ...
        return np.array(currents) , np.array(parameters), cell_li
